...
import { bootstrap, Harness, newUser, onboard, resetDb, startHarness, TestUser } from '../helpers/harness';
...
function localAiMatch(seed: number, aiSeed: number, difficulty: 'easy' | 'normal' | 'hard') {
  ...
  for (let i = 0; i < 3000 && state.status === 'PLAYING'; i++) {
    const botAction = nextAIAction(state, seats, rng);
    let action: GameAction | null = botAction;
    if (!action && getAvailableActions(state, 0).length > 0) action = timeoutAction(viewForSeat(state, 0), 0);
...
describe('Liga, matchmaking e partida contra IA', () => {
  ...
  const idOf = async (u: TestUser) => (await h.prisma.user.findUniqueOrThrow({ where: { firebaseUid: u.uid } })).id;
  ...
  it('tela da liga: snapshot completo com zonas e ranking do servidor', async () => {
    ...
    await leagues.addWeeklyPoints({ userId: ids[3]!, matchKey: 'm1', event: LeagueScoreEvent.MATCH_WIN, points: 25, won: true });
    ...
    const ranking = await h.http.get(`/v1/leagues/groups/${snap.groupId}/ranking`).set('authorization', users[1]!.auth).expect(200);
    expect(ranking.body.data.members).toHaveLength(6);
    const global = await h.http.get('/v1/leagues/ranking/global?limit=3').set('authorization', users[0]!.auth).expect(200);
    expect(global.body.data.entries[0]).toMatchObject({ uid: users[3]!.uid, seasonPoints: 25, rank: 1 });
  ...
  it('pontos da semana são idempotentes por partida + usuário + evento', async () => {
    ...
    const input = { userId: id, matchKey: 'dup-match', event: LeagueScoreEvent.MATCH_WIN, points: 25, won: true };
    const results = await Promise.all([leagues.addWeeklyPoints(input), leagues.addWeeklyPoints(input), leagues.addWeeklyPoints(input)]);
  ...
  it('virada semanal: promoção, rebaixamento, histórico e próxima semana — job idempotente', async () => {
    ...
    for (const [i, id] of ids.entries())
      if (i < 7)
        await leagues.addWeeklyPoints({ userId: id, matchKey: `w${i}`, event: LeagueScoreEvent.MATCH_WIN, points: 100 - i * 10, won: true });
    ...
    expect(top).toMatchObject({ currentLeagueId: 'gold', lastWeeklyResult: 'PROMOTED', lastWeeklyRank: 1, weeklyPoints: 0 });
    ...
    const history = await h.http.get('/v1/leagues/me/history').set('authorization', users[0]!.auth).expect(200);
    expect(history.body.data.items[0]).toMatchObject({ weekKey: week, result: 'promoted', finalRank: 1, nextLeagueId: 'gold' });
    ...
    const g = await h.prisma.leagueGroup.findFirstOrThrow({ where: { weekKey: next, leagueId: 'gold' } });
  ...
  it('rebalanceamento: 30 jogadores na mesma liga viram dois grupos de 15', async () => {
    ...
    for (const u of users) await onboard(h, u);
    const groups = await h.prisma.leagueGroup.findMany({ where: { status: { in: ['ACTIVE', 'FORMING'] } } });
  ...
  it('matchmaking: 4 jogadores formam mesa; com IA um jogador sozinho joga', async () => {
    ...
    const entries = await Promise.all(
      users.map(async (u) => (await h.http.get('/v1/matchmaking').set('authorization', u.auth).expect(200)).body.data.entry),
    );
    expect(entries.every((e) => e.status === 'ready' && e.sessionId === entries[0].sessionId)).toBe(true);
    const parts = await h.prisma.matchParticipant.findMany({ where: { matchId: entries[0].sessionId } });
    ...
    expect((await h.http.get('/v1/matchmaking').set('authorization', solo.auth)).body.data.entry.status).toBe('cancelled');
    await h.http.post('/v1/matchmaking').set('authorization', solo.auth).send({ allowBots: true }).expect(200);
    const entry = (await h.http.get('/v1/matchmaking').set('authorization', solo.auth)).body.data.entry;
    expect(entry.status).toBe('ready');
    const soloParts = await h.prisma.matchParticipant.findMany({ where: { matchId: entry.sessionId } });
  ...
  it('matchmaking: busca abandonada (app morto) expira e não senta ninguém numa mesa', async () => {
    ...
    for (const u of users) await onboard(h, u);
    for (const u of users.slice(0, 3)) await h.http.post('/v1/matchmaking').set('authorization', u.auth).send({}).expect(200);
    ...
    const statuses = await Promise.all(
      users.map(async (u) => (await h.http.get('/v1/matchmaking').set('authorization', u.auth)).body.data.entry.status),
    );
  ...
  it('partida contra IA: o servidor re-executa, pontua uma vez e recusa fraude', async () => {
    const u = newUser('ai');
    await onboard(h, u);
    const { state, actions } = localAiMatch(12345, 777, 'hard');
    const body = { matchId: 'local-match-0001', seed: 12345, aiSeed: 777, difficulty: 'hard', actions };
    const res = await h.http.post('/v1/matches/ai').set('authorization', u.auth).send(body).expect(200);
    ...
    const again = await h.http.post('/v1/matches/ai').set('authorization', u.auth).send(body).expect(200);
