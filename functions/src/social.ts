...
export const sendFriendRequest = authedCallable<{ toUid: string }, { ok: true }>(
  async ({ uid, data }) => {
    ...
    const [me, target, friendship] = await Promise.all([
    ...
    const pending = await db
      ...
      .limit(1)
      .get();
    if (!pending.empty) throw new HttpsError('already-exists', 'Solicitação já enviada.');
    ...
    await db.collection('friendRequests').add(req);
    await pushTo(
      data.toUid,
